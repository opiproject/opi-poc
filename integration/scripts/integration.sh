#!/bin/bash
# SPDX-License-Identifier: Apache-2.0
# Copyright (c) 2022 Intel Corporation

INT_BUILD_START=build-start
INT_RUN_TESTS=run-tests
INT_LOGS=logs
INT_STOP=stop

# TARGET is either 'dpu' or 'ipu'
INT_TARGET=dpu

# DOCKER_COMPOSE setup
DC=docker-compose

if [ "$(which ${DC})" == "" ]
then
    DC="docker compose"
fi

usage() {
    echo ""
    echo "Usage: integration.sh -a [dpu | ipu] [${INT_BUILD_START} | ${INT_RUN_TESTS} | ${INT_LOGS} | ${INT_STOP}]"
    echo ""
}

build_and_start_containers() {
    bash -c "${DC} -f docker-compose.yml -f docker-compose.${INT_TARGET}.yml -f docker-compose.otel.yml -f docker-compose.spdk.yml -f docker-compose.pxe.yml down"
    docker network prune --force
    bash -c "${DC} -f docker-compose.yml -f docker-compose.${INT_TARGET}.yml -f docker-compose.otel.yml -f docker-compose.spdk.yml -f docker-compose.pxe.yml up -d"
}

run_integration_tests() {
    bash -c "${DC} -f docker-compose.yml -f docker-compose.${INT_TARGET}.yml -f docker-compose.otel.yml -f docker-compose.spdk.yml -f docker-compose.pxe.yml ps"
    echo wait 5s... && sleep 5s
    curl --fail http://127.0.0.1:8001/redfish/v1/Systems/437XR1138R2
    curl --fail http://127.0.0.1:8002/redfish/v1/Systems/437XR1138R2
    curl --fail http://127.0.0.1:8082/var/lib/tftpboot/
    curl --fail http://127.0.0.1:9090/
    curl --fail --insecure --user spdkuser:spdkpass -X POST -H 'Content-Type: application/json' -d '{\"id\": 1, \"method\": \"bdev_get_bdevs\"}' http://127.0.0.1:9004
    curl --fail --insecure --user spdkuser:spdkpass -X POST -H 'Content-Type: application/json' -d '{\"id\": 1, \"method\": \"bdev_get_bdevs\"}' http://127.0.0.1:9009
    bash -c "${DC} -f docker-compose.pxe.yml exec -T pxe dnf install -y nmap tftp"
    bash -c "${DC} -f docker-compose.pxe.yml exec -T pxe nmap --script broadcast-dhcp-discover"
    bash -c "${DC} -f docker-compose.pxe.yml exec -T pxe nmap --script broadcast-dhcp-discover | grep "Server Identifier: 10.127.127.3""
    bash -c "${DC} -f docker-compose.pxe.yml exec -T pxe curl --fail http://10.127.127.3:8082/var/lib/tftpboot/"
    bash -c "${DC} -f docker-compose.pxe.yml exec -T pxe tftp 10.127.127.3 -v -c get grubx64.efi"
}

acquire_logs() {
    bash -c "${DC}" logs || true
    netstat -an || true
    ifconfig -a || true
    docker inspect bash -c "${DC} -f docker-compose.yml -f docker-compose.${INT_TARGET}.yml -f docker-compose.otel.yml -f docker-compose.spdk.yml -f docker-compose.pxe.yml ps -aq" || true
}

stop_containers() {
    bash -c "${DC} -f docker-compose.yml -f docker-compose.${INT_TARGET}.yml -f docker-compose.otel.yml -f docker-compose.spdk.yml -f docker-compose.pxe.yml down"
}

# Process commandline arguments
while getopts a: flag
do
    case "${flag}" in
        a)
            INT_TARGET="${OPTARG}"
            shift 2
            ;;
        *)
            usage
            exit 1
            ;;
    esac
done

# Verify architecture
if [ "${INT_TARGET}" != "dpu" ] && [ "${INT_TARGET}" != "ipu" ]
then
    usage
    exit 1
fi

if [ "$#" -lt 1 ]
then
    usage
    exit 1
fi

if [ "$1" == "${INT_BUILD_START}" ]
then
    build_and_start_containers
elif [ "$1" == "${INT_RUN_TESTS}" ]
then
    run_integration_tests
elif [ "$1" == "${INT_LOGS}" ]
then
    acquire_logs
elif [ "$1" == "${INT_STOP}" ]
then
    stop_containers
else
    echo "Invalid argument: $1"
    usage
    exit 1
fi
