...
import (
	...
	"evpn-gw-cni/pkg/config"
	"evpn-gw-cni/pkg/sriov"
...
func deleteResources(args *skel.CmdArgs, netConf *evpngwtypes.NetConf) error {
	var err error
...
func cmdAdd(args *skel.CmdArgs) error {
	var macAddr string
	var err error
	var netConf *evpngwtypes.NetConf
	...
	if err != nil {
		return fmt.Errorf("evpn-gw-cni failed to load netconf: %v", err)
	}

	envArgs, err := getEnvArgs(args.Args)
	if err != nil {
		return fmt.Errorf("evpn-gw-cni failed to parse args: %v", err)
	}
	...
	if err = sm.ApplyVFConfig(netConf); err != nil {
		return fmt.Errorf("evpn-gw-cni failed to configure VF %q", err)
	}
